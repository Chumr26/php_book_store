<div class="container-fluid">
    ...
    <div class="row">
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <?php render_admin_component('stat_card', [
                'title' => 'Doanh thu',
                'value' => number_format($statistics['total_revenue'] ?? 0) . 'đ',
                'subtitle' => 'Giá trị đơn TB: ' . number_format($statistics['avg_order_value'] ?? 0) . 'đ',
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <?php render_admin_component('stat_card', [
                'title' => 'Đơn hàng',
                'value' => number_format($statistics['total_orders'] ?? 0),
                'subtitle' => 'Tỷ lệ chuyển đổi: ' . number_format($statistics['conversion_rate'] ?? 0, 2) . '%',
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <?php render_admin_component('stat_card', [
                'title' => 'Thành công',
                'value' => number_format($statistics['success_orders'] ?? 0),
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <?php render_admin_component('stat_card', [
                'title' => 'Chờ xử lý',
                'value' => number_format($statistics['pending_orders'] ?? 0),
...
<script>
        ...
        data: {
            labels: <?php
                    $labels = array_map(function ($item) {
                        return $item['date'] ?? $item['hour'] ?? $item['month'];
                    }, $revenue_chart);
    ...
    if (newCustomersCanvas) {
        var newCustomersLabels = <?php
                                    $customerLabels = array_map(function ($item) {
                                        return $item['date'] ?? $item['hour'] ?? $item['month'];
                                    }, $new_customers_chart ?? []);
                                    echo json_encode($customerLabels);
                                    ?>;
        var newCustomersValues = <?php
                                    $customerValues = array_map(function ($item) {
    ...
    if (topSellingCanvas) {
        var topSellingLabels = <?php
                                ...
                                $topLabels = array_map(function ($book) {
                                    return $book['ten_sach'] ?? '';
                                }, $topBooks);
                                echo json_encode($topLabels);
                                ?>;
        var topSellingValues = <?php
                                $topValues = array_map(function ($book) {
                                    return $book['total_sold'] ?? 0;
                                }, $topBooks);
